...
def mark_lora_and_gate_as_trainable(model: nn.Module, bias: str = 'none') -> None:
    for n, p in model.named_parameters():
        if 'lora_' not in n and 'gate.weight' not in n:
            p.requires_grad = False
        else:
            p.requires_grad = True
...
def get_lora_and_gate_parameters(model, bias='none'):
    params = []
