...
def evaluate_lora(args, clip_model, loader, dataset):
    ...
    with torch.no_grad():
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
            ...
            output = clip_model.encode_text(texts)
            if isinstance(output, tuple):
                class_embeddings = output[0]
            else:
        ...
        for i, (images, target) in enumerate(tqdm(train_loader)):
            ...
            if args.encoder == 'text' or args.encoder == 'both':
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    output = clip_model.encode_text(texts)
                    if isinstance(output, tuple):
                        class_embeddings, load_balancing_loss = output
                        total_loss += load_balancing_loss * args.load_balancing_coef
                    else:
                        class_embeddings = output
                text_features = class_embeddings/class_embeddings.norm(dim=-1, keepdim=True)
            ...
            if args.encoder == 'vision' or args.encoder == 'both':
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    output = clip_model.encode_image(images)
                    if isinstance(output, tuple):
                        image_features, load_balancing_loss = output
                        total_loss += load_balancing_loss * args.load_balancing_coef
                    else:
                        image_features = output
            else:
